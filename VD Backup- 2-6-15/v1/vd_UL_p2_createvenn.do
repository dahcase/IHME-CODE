...
*Behavioral Risks: Level 4 --note: smoking is in here but may not be a level four factor
...
local combos all beh_metab env_metab env_beh env metab beh 
...
//open the file
	...
	*Drop the aggregated age groups which I can recalculate later
	if "`a11'"=="Y" {
		drop if sex==3
		drop if (age>80 & age !=98) //this keeps the age standardized which I cannot recreate
	}
	...
	keep acause iso3 age year sex risk lower_cf_death lower_cf_yld
	
	
	rename lower_cf_death mt
	rename lower_cf_yld mb
...
if "`a2'"== "Y"{
	...
	//note: rewrite so that this is not a hard fix
		...
		merge m:1 sex year acause age using "$prefix/WORK/05_risk/other/venn/pafsone/pafsone.dta", nogen keep(1 3) keepusing(risk draw* acause)
...
if "`a5'" == "Y" {
	...
	replace mean_yll=0 if mean_yll==.
	replace mean_yld=0 if mean_yld==.
	...
	*DROP CAUSES WITHOUT A RISK FACTOR PAF (e.g. all 0s)
	...
	if "`a11'"=="Y" {
		if `useintermediate'==1 {
			save "`workdirectory'/pre_aggtest_`countryname'_`fileyear'.dta", replace

		}
		destring age, replace
		/* this is done up above
		drop if sex==3
		drop if (age>80 & age !=98) //this keeps the age standardized which I cannot recreate
		*/
		//generate the aggregate sex groups
		fastcollapse mean* death* yll* yld* daly*, type(sum) by(iso3 year age cause acause) append flag(dup)
		...
		fastcollapse mean* death* yll* yld* daly*, type(sum) by(iso3 year sex cause acause) append flag(dup)
		...
		fastcollapse mean* death* yll* yld* daly*, type(sum) by(iso3 year newage sex cause acause) append flag(dup)
	...
	if `useintermediate'==1 {
		...
		*merge m:1 union_type using "$prefix/WORK/05_risk/other/venn/dimensions/union_type.dta", keepusing(union_type_id) keep(3) nogen
		...
		if `subnatck'>3 {
			...
			if strpos(local_id, "IMD_")>0 {
				replace local_id = "`countryname'"
			}
