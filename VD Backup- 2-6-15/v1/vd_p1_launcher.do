...
local vdversion `dalynatversion'_nore //version of the venn diagram
local codeversion 1 //version of the code
local medversion 1 //version of the mediation
//////////Stage #:12345678901
local instruction YYYYYYYYNNN //foreach stage, say whether you want to run this part (e.g. a y or n in the appropriate part)
...
foreach file of local sumfiles {
	...
	*!/usr/local/bin/SGE/bin/lx24-amd64/qsub -N `job' -pe multi_slot 4 -l mem_free=4G "`codedirectory'stata_shell.sh" "`codedirectory'v`codeversion'/vd_p2_createvenn.do" "`thefile' `countryname' `fileyear' `codeversion' `dalynatversion' `vdversion' `instruction'"
	...
	if c(os) == "Unix" {
		...
		do "`codedirectory'v`codeversion'/vd_p2_createvenn.do" `thefile' `countryname' `fileyear' `codeversion' `dalynatversion' `vdversion' `medversion' `workdirectory' `instruction'
	}
}
