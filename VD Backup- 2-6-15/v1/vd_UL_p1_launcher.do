...
local dalynatversion 65 //version of the dalynator
local vdversion `dalynatversion'_lower //version of the venn diagram
local codeversion 1 //version of the code
local medversion 1 //version of the mediation
//////////Stage #:12345678901
local instruction YYYYYYYYNNY //foreach stage, say whether you want to run this part (e.g. a y or n in the appropriate part)
					   //each letter refers to a stage
***NOTE: local run is set in the define jdrive area

*Contraints
local location USA //MD1 MD2 MD3 MD4 MD5 //if you want to constrain the run by country(s) replace all with the iso3 codes.
...
local years 2010 //1990 1995 2000 2005 2010 //constrain the years that the process runs.
...
cap mkdir "`workdirectory'/dimensions/hold"
...
foreach file of local sumfiles {
	...
	*!/usr/local/bin/SGE/bin/lx24-amd64/qsub -N `job' -pe multi_slot 4 -l mem_free=4G "`codedirectory'stata_shell.sh" "`codedirectory'v`codeversion'/vd_p2_createvenn.do" "`thefile' `countryname' `fileyear' `codeversion' `dalynatversion' `vdversion' `instruction'"
	...
	if c(os) == "Unix" {
		!/usr/local/bin/SGE/bin/lx24-amd64/qsub -N `job' -pe multi_slot 4 -l mem_free=4G "`codedirectory'stata_shell.sh" "`codedirectory'v`codeversion'/vd_p2_createvenn.do" "`thefile' `countryname' `fileyear' `codeversion' `dalynatversion' `vdversion' `medversion' `workdirectory' `instruction'"
		local thejobs `thejobs'`job',
		sleep 100
	}
	else if c(os) == "Windows" {
		do "`codedirectory'v`codeversion'/vd_UL_p2_createvenn.do" `thefile' `countryname' `fileyear' `codeversion' `dalynatversion' `vdversion' `medversion' `workdirectory' `instruction'
	}
}
...
if `run'==1 {
	...
	if c(os) == "Unix" {
		...
		do "`codedirectory'v`codeversion'/vd_p3_makedimensions.do" `workdirectory'
	}
