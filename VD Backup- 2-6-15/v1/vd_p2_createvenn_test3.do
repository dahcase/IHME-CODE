...
qui do J:\WORK\05_risk\03_outputs\01_code\02_paf_calculator\functions_agg_paf.ado
...
if "`a2'"== "Y"{
	...
	foreach combo of local combos{
		...
		*if a one way or the all aggregation
		if `under'==0 {
			keep if strpos(agg_risk_vd, "`combo'")>0 | "`combo'"=="all"
		}
		else {
		...
		if `iter'==0{
			...
			merge 1:1 iso3 year age acause sex using `riskholder',keep(3) nogen
			save `riskholder', replace
		}
