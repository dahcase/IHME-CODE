...
if c(os) == "Unix" {
	...
	local run 0 //1 means run the dimensions creation process as well
}
...
//////////Stage #:12345678901
local instruction YYYYYYNNNNY //foreach stage, say whether you want to run this part (e.g. a y or n in the appropriate part)
					   //each letter refers to a stage
***NOTE: local run is set in the define jdrive area

*Contraints
local location CHL USA CAF DEU //if you want to constrain the run by country(s) replace all with the iso3 codes.
...
local years 2010 //constrain the years that the process runs.
...
foreach file of local sumfiles {
	...
	*!/usr/local/bin/SGE/bin/lx24-amd64/qsub -N `job' -pe multi_slot 4 -l mem_free=4G "`codedirectory'stata_shell.sh" "`codedirectory'v`codeversion'/vd_p2_createvenn.do" "`thefile' `countryname' `fileyear' `codeversion' `dalynatversion' `vdversion' `instruction'"
	...
	if c(os) == "Unix" {
		!/usr/local/bin/SGE/bin/lx24-amd64/qsub -N `job' -pe multi_slot 4 -l mem_free=4G "`codedirectory'stata_shell.sh" "`codedirectory'v`codeversion'/vd_p2_createvenn_test2.do" "`thefile' `countryname' `fileyear' `codeversion' `dalynatversion' `vdversion' `medversion' `workdirectory' `instruction'"
		local thejobs `thejobs'`job',
		sleep 100
	}
	else if c(os) == "Windows" {
		do "`codedirectory'v`codeversion'/vd_p2_createvenn_test2.do" `thefile' `countryname' `fileyear' `codeversion' `dalynatversion' `vdversion' `medversion' `workdirectory' `instruction'
	}
}
