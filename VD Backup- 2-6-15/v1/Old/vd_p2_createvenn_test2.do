	...
	foreach combo of local combos {
		...
		*save "`workdirectory'/medmid_`combo'_`countryname'_`fileyear'.dta", replace
		...
		if "`a11'"=="Y" {
			...
			drop if age>90
			tostring age, replace
			//drop if sex==3 --- this looks ok for right now
		}
...
if "`a6'" == "Y" {
	...
	if "`a11'"=="Y" {
		...
		//forget sex for right now. Come back to this later after more testing
		*gen newsex=3
		...
		*fastcollapse age sex cause iso3 year union_type measure if cause_level == `i', type(sum) by(iso3 year age sex risk cause_parent) append flag(dup)
		...
		//age group 91
		destring age, replace
		fastcollapse value if age==5 | age==10, type(sum) by(sex acause cause iso3 year union_type measure) append flag(dup)
		replace age=91 if dup==1
		drop dup
		
		fastcollapse value if age>=5 | age<=40, type(sum) by(sex acause cause iso3 year union_type measure) append flag(dup)
		replace age=92 if dup==1
		drop dup
		
		fastcollapse value if age>=50 | age<=65, type(sum) by(sex acause cause iso3 year union_type measure) append flag(dup)
		replace age=93 if dup==1
		drop dup
		
		fastcollapse value if age>=70 | age<=80, type(sum) by(sex acause cause iso3 year union_type measure) append flag(dup)
		replace age=94 if dup==1
		drop dup
		
		fastcollapse value if age>=0 | age<=80, type(sum) by(sex acause cause iso3 year union_type measure) append flag(dup)
		replace age=99 if dup==1
		drop dup
		
		fastcollapse value if age>=0| age<1, type(sum) by(sex acause cause iso3 year union_type measure) append flag(dup)
		replace age=95 if dup==1
		drop dup
	}
